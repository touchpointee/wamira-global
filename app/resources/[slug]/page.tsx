    ...
    <>
      <article>
        <section className="relative min-h-[560px] overflow-hidden pt-32">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-teal-dark via-teal-dark/88 to-teal-dark/45" />
          <div className="absolute inset-0 bg-luxury-radial" />
          <div className="relative mx-auto max-w-5xl px-5 pb-20 pt-24">
            <Link href="/resources" className="mb-8 inline-flex items-center gap-2 text-sm font-semibold text-gold-champagne transition hover:text-white">
              <ArrowLeft className="h-4 w-4" />
              Back to Resources
            </Link>
            <div className="mb-5 flex flex-wrap items-center gap-3 text-xs font-bold uppercase tracking-[0.22em] text-gold-champagne">
              <span>{article.category}</span>
              <span className="h-px w-10 bg-gold-champagne/50" />
              <span>{article.date}</span>
            </div>
            <h1 className="font-serif text-5xl leading-tight text-white md:text-7xl">{article.title}</h1>
            <p className="mt-7 max-w-3xl text-lg leading-8 text-white/80">{article.excerpt}</p>
          </div>
        </section>

        <section className="mx-auto max-w-4xl px-5 py-20">
          <div className="rounded border border-black/5 bg-white p-6 shadow-dark md:p-10">
